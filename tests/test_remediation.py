...
class RemediationManagerTests(unittest.TestCase):
    ...
    def test_detect_unresolved_findings(self) -> None:
        f1 = NormalizedFinding(
            id="1", source="ocr", severity="high", category="security", file="src/a.py", message="SQLi"
        )
        f2 = NormalizedFinding(
            id="2", source="ocr", severity="medium", category="maintainability", file="src/b.py", message="Style"
        )
        f3_resolved = NormalizedFinding(
            id="3", source="ocr", severity="low", category="testing", file="src/c.py", message="Unused"
        )
