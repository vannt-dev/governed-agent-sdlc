...
class OpenCodeReviewProviderTests(unittest.TestCase):
    def test_missing_executable_raises_error(self) -> None:
        provider = OpenCodeReviewProvider(executable="nonexistent_ocr_bin_xyz_999")
        self.assertFalse(provider.is_available())
        with self.assertRaises(FileNotFoundError):
            provider.review(ReviewContext(run_id="r1", repository_root=Path(".")))

    def test_successful_ocr_execution(self) -> None:
        from unittest.mock import MagicMock, patch

        provider = OpenCodeReviewProvider(executable="mock_ocr")
        mock_proc = MagicMock()
        ...
        with patch("shutil.which", return_value="/bin/mock_ocr"), patch(
            ...
            result = provider.review(
                ReviewContext(
                    ...
                    files=("src/api.py",),
                    background_file="/repo/docs/spec.md",
                )
            )

            mock_run.assert_called_once()
            cmd_args = mock_run.call_args[0][0]
            self.assertIn("--files", cmd_args)
            self.assertIn("src/api.py", cmd_args)
            self.assertIn("--context", cmd_args)
            self.assertIn("/repo/docs/spec.md", cmd_args)

            self.assertEqual(result.provider, "open-code-review")
            self.assertFalse(result.passed)
            self.assertEqual(len(result.findings), 1)
            self.assertEqual(result.findings[0].severity, "high")
            self.assertEqual(result.findings[0].category, "security")

    def test_ocr_timeout_handling(self) -> None:
        import subprocess
        from unittest.mock import patch

        provider = OpenCodeReviewProvider(executable="mock_ocr", timeout_seconds=5)
        with patch("shutil.which", return_value="/bin/mock_ocr"), patch(
            "subprocess.run", side_effect=subprocess.TimeoutExpired(cmd="mock_ocr", timeout=5)
        ):
            result = provider.review(ReviewContext(run_id="r3", repository_root=Path(".")))
            self.assertFalse(result.passed)
            self.assertEqual(len(result.findings), 1)
            self.assertEqual(result.findings[0].id, "ocr-timeout")
